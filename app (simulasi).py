...
def _choose_realisasi_column(columns: list[str], target: str) -> Optional[str]:
    ...
    target_norm = target_norm.replace(".", "").replace("-", "_")
    for col in columns:
...
def _build_growth_table(level_df: pd.DataFrame, periods: int, growth_name: str) -> pd.DataFrame:
    ...
    for indikator in PDB_COMPONENTS:
        ...
        # Full year growth based on annual sums if data exists
        s["yearly_sum"] = s.groupby("tahun")[indikator].transform("sum")
        annual = s[["tahun", "yearly_sum"]].drop_duplicates().sort_values("tahun")
...
def derive_pdb_from_realisasi(source: Union[str, bytes]):
    ...
    level_df = raw[["tanggal", *mapping.values()]].copy()
    rename_back = {v: k for k, v in mapping.items()}
    level_df = level_df.rename(columns=rename_back)
    for indikator in PDB_COMPONENTS:
        if indikator not in level_df.columns:
            level_df[indikator] = None
    level_df = level_df[["tanggal", *PDB_COMPONENTS]].copy()

    nominal_table = _build_period_table_from_series_map(level_df, {k: k for k in PDB_COMPONENTS})
    nominal_table = coerce_schema(nominal_table, "pdb")
    yoy_table = _build_growth_table(level_df, periods=4, growth_name="yoy")
    yoy_table = coerce_schema(yoy_table, "pdb")
    qtq_table = _build_growth_table(level_df, periods=1, growth_name="qtq")
    qtq_table = coerce_schema(qtq_table, "pdb")
    ...
    history_bundle = {"level": hist_long, "growth": growth_long}
    tables_bundle = {"nominal": nominal_table, "yoy": yoy_table, "qtq": qtq_table}
    return nominal_table, history_bundle, tables_bundle, level_df
...
def render_table_block(block_df: pd.DataFrame, accent: bool = False, block_key: str = "", chart_key: Optional[str] = None):
    st.plotly_chart(
        ...
        config=TABLE_CONFIG,
        key=chart_key or f"table_{block_key or 'default'}",
    )
...
def render_growth_table(df: pd.DataFrame, title: str, header_fill: str, chart_key: Optional[str] = None):
    ...
    st.plotly_chart(
        ...
        use_container_width=True,
        config=TABLE_CONFIG,
        key=chart_key or f"growth_{title.lower().replace(' ', '_')}",
    )
...
def render_simulasi_fiskal_editor() -> pd.DataFrame:
    st.markdown(
        '<div class="fiscal-editor-header"><div class="fiscal-editor-title">SIMULASI FISKAL</div><div class="fiscal-editor-unit">dalam Triliun</div></div>',
        unsafe_allow_html=True,
    )
...
render_table_block(workbook["pdb"], block_key="pdb", chart_key="main_pdb_nominal_2026")

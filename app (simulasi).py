...
SIMULASI_FISKAL_TITLE = "SIMULASI FISKAL (ON TOP)"
...
def build_simulasi_fiskal_df() -> pd.DataFrame:
    return pd.DataFrame({
        ...
        "full_year": [0.0] * len(SIMULASI_FISKAL_ROWS),
    })


def render_simulasi_fiskal_panel(df: pd.DataFrame | None = None):
    ...
    sim_df["indikator"] = sim_df["indikator"].fillna("")
    for col in expected_cols[1:]:
        sim_df[col] = pd.to_numeric(sim_df[col], errors="coerce").fillna(0.0)

    html = [
        '<div class="fiscal-sim-wrapper">',
        f'<div class="fiscal-sim-title">{SIMULASI_FISKAL_TITLE}<br>(ON TOP)</div>',
        '<div class="fiscal-sim-grid">',
        '<div></div>',
        '<div class="fiscal-sim-head">Q1</div>',
        '<div class="fiscal-sim-head">Q2</div>',
        '<div class="fiscal-sim-head">Q3</div>',
        '<div class="fiscal-sim-head">Q4</div>',
    ]

    for _, row in sim_df.iterrows():
        html.append(f'<div class="fiscal-sim-label">{row["indikator"]}</div>')
        for col in ["out_tw1", "out_tw2", "out_tw3", "out_tw4"]:
            html.append(f'<div class="fiscal-sim-val">{fmt_id0(row[col])}</div>')

    html.append('</div>')
    html.append('<div class="fiscal-sim-foot">dalam Rp T</div>')
    html.append('</div>')
...
render_simulasi_fiskal_panel(simulasi_fiskal_df)

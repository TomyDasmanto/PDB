...
st.markdown(
    ...
    <style>
        ...
        .fiscal-sim-table col:first-child {{
            width: 47%;
        }}
        .fiscal-sim-table col:not(:first-child) {{
            width: 13.25%;
        }}
        .fiscal-sim-table th,
        .fiscal-sim-table td {{
        ...
        .fiscal-sim-title-cell {{
            color: #3B6EC2;
            font-size: 1.05rem;
            font-weight: 800;
            text-align: left;
        }}
        .fiscal-sim-head-cell {{
            color: #000000;
            font-size: 0.98rem;
            font-weight: 800;
            text-align: center;
        }}
        .fiscal-sim-label-cell {{
            color: #3B6EC2;
            font-size: 0.98rem;
            font-weight: 800;
            text-align: left;
...
def render_simulasi_fiskal_table(df: Optional[pd.DataFrame] = None):
    sim_df = build_simulasi_fiskal_df() if df is None else df.copy()
    expected_cols = ["indikator", "out_tw1", "out_tw2", "out_tw3", "out_tw4"]
    for col in expected_cols:
        if col not in sim_df.columns:
            sim_df[col] = 0.0 if col != "indikator" else SIMULASI_FISKAL_ROWS

    sim_df = sim_df[expected_cols].copy()
    sim_df["indikator"] = sim_df["indikator"].fillna("").astype(str)
    for col in expected_cols[1:]:
        sim_df[col] = pd.to_numeric(sim_df[col], errors="coerce").fillna(0.0)

    html = [
        '<div class="fiscal-sim-box">',
        '<div class="fiscal-sim-unit">dalam Triliun</div>',
        '<table class="fiscal-sim-table">',
        '<colgroup><col><col><col><col><col></colgroup>',
        '<thead>',
        '<tr>',
        '<th class="fiscal-sim-title-cell">SIMULASI FISKAL</th>',
        '<th class="fiscal-sim-head-cell">Q1</th>',
        '<th class="fiscal-sim-head-cell">Q2</th>',
        '<th class="fiscal-sim-head-cell">Q3</th>',
        '<th class="fiscal-sim-head-cell">Q4</th>',
        '</tr>',
        '</thead>',
        '<tbody>',
    ]
    ...
    for _, row in sim_df.iterrows():
        html.append('<tr>')
        html.append(f'<td class="fiscal-sim-label-cell">{row["indikator"]}</td>')
        for col in ["out_tw1", "out_tw2", "out_tw3", "out_tw4"]:
            html.append(f'<td class="fiscal-sim-value-cell">{fmt_id0(row[col])}</td>')
        html.append('</tr>')
...
simulasi_fiskal_df = build_simulasi_fiskal_df()
...
render_simulasi_fiskal_table(simulasi_fiskal_df)

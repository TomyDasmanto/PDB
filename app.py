...
def derive_pdb_from_realisasi(source: Union[str, bytes]):
    ...
    return table, {"level": hist_long, "growth": growth_df}
...
def load_dashboard_data():
    ...
    if source is None:
        return data, pdb_history, source_status
    ...
    try:
        ...
        if "realisasi" in lower_sheet_map:
            data["pdb"], pdb_history = derive_pdb_from_realisasi(source)
        ...
        return data, pdb_history, f"Gagal membaca sumber Excel otomatis: {e}"
...
def make_growth_chart(pdb_history: Optional[dict], selected_components: list[str], growth_col: str, title: str):
...
with tab_pdb:
    ...
    st.markdown("<div class='chart-note'>Format angka dashboard sudah menggunakan gaya Indonesia tanpa desimal. Grafik historis level menampilkan nilai asli, sedangkan dua grafik tambahan menampilkan pertumbuhan Year on Year (YoY) dan Quarter to Quarter (QtQ).</div>", unsafe_allow_html=True)
    ...
    with ch2:
        st.plotly_chart(
            make_growth_chart(pdb_history, selected_components, "yoy", "Pertumbuhan Year on Year (YoY)"),
            use_container_width=True,
            config={"displayModeBar": True, "displaylogo": False},
        )
    with ch3:
        st.plotly_chart(
            make_growth_chart(pdb_history, selected_components, "qtq", "Pertumbuhan Quarter to Quarter (QtQ)"),
            use_container_width=True,
            config={"displayModeBar": True, "displaylogo": False},
        )
    st.markdown('</div>', unsafe_allow_html=True)

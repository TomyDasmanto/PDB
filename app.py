...
def compute_column_widths(view: pd.DataFrame, min_px: int = 76, max_px: int = 260) -> list[int]:
    widths = []
    for idx, col in enumerate(view.columns):
        ...
        if idx == 0:
            width = min(max(138, 7 * max_len + 22), max_px)
        else:
            width = min(max(min_px, 6 * max_len + 16), 118)

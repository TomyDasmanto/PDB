...
def make_growth_table(df: pd.DataFrame, header_fill: str, first_col_width=230, other_col_width=112, height=260, value_formatter=fmt_pct_id2):
    ...
    fill_matrix = []
    # indikator column
    indicator_colors = [CELL_GREEN_A if i % 2 == 0 else CELL_GREEN_B for i in range(len(view))]
    fill_matrix.append(indicator_colors)
    # numeric period columns with sign colors
    for col in PERIOD_ORDER:
...
def make_pdb_history_chart(pdb_history: Optional[dict], selected_components: list[str]):
    ...
    fig = px.line(
        ...
        color="komponen",
        markers=True,
        color_discrete_sequence=[PRIMARY, ACCENT, SUCCESS, PURPLE, NEGATIVE, "#F4A261"],
...
def make_growth_chart(pdb_history: Optional[dict], selected_components: list[str], growth_col: str, title: str, colors=None):
    ...
    fig = px.line(
        ...
        color="komponen",
        markers=True,
        color_discrete_sequence=colors or [PRIMARY, ACCENT, SUCCESS, PURPLE, NEGATIVE, "#F4A261"],
    ...
    with ch2:
        st.plotly_chart(
            make_growth_chart(
                ...
                colors=[SUCCESS, ACCENT, PRIMARY, PURPLE, NEGATIVE, "#F4A261"],
            ),
            use_container_width=True,
            config={"displayModeBar": True, "displaylogo": False},
        )
    with ch3:
        st.plotly_chart(
            make_growth_chart(
                ...
                colors=[PURPLE, SUCCESS, PRIMARY, ACCENT, NEGATIVE, "#F4A261"],
            ),
            use_container_width=True,
            config={"displayModeBar": True, "displaylogo": False},
        )

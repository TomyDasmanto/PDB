...
# Opsional: isi raw URL GitHub lewat st.secrets['github_raw_xlsx_url']
...
CELL_GREEN_A = "#DCEFEA"
CELL_GREEN_B = "#EAF6F3"
CELL_PURPLE_A = "#E7E0F7"
CELL_PURPLE_B = "#F1ECFB"
BG = "#F6F7FB"
TEXT = "#1F2937"
...
BLOCK_NOTES = {
    ...
    "pdb": "Accounting / PDB dibaca otomatis dari file GitHub `dashboard PDB.xlsx`. Selain tabel nominal 2026, ditambahkan juga tabel pertumbuhan Year on Year (YoY) dan Quarter to Quarter (QtQ) dengan struktur periode yang sama.",
...
def make_tick_values_pct(series: pd.Series, n: int = 6):
    ...
    return vals, [fmt_pct_id0(v) for v in vals]
...
def derive_pdb_from_realisasi(source: Union[str, bytes]):
    ...
    for comp in PDB_COMPONENTS:
        ...
        if comp in data_2026.columns:
            ...
            # QtQ full year tidak baku; dibiarkan kosong agar tidak menyesatkan
            qtq["full_year"] = None
...
def make_pdb_history_chart(pdb_history: Optional[dict], selected_components: list[str]):
    ...
    fig = px.line(
        ...
        color="komponen",
        markers=True,
        color_discrete_sequence=[PRIMARY, ACCENT, SUCCESS, PURPLE, "#D14D72", "#F4A261"],
...
def make_growth_chart(pdb_history: Optional[dict], selected_components: list[str], growth_col: str, title: str, colors=None):
    ...
    plot_df["growth_fmt"] = plot_df[growth_col].apply(fmt_pct_id0)
    fig = px.line(
        plot_df,
        x="tanggal",
        y=growth_col,
        color="komponen",
        markers=True,
        color_discrete_sequence=colors or [PRIMARY, ACCENT, SUCCESS, PURPLE, "#D14D72", "#F4A261"],
...
def render_growth_table(df: pd.DataFrame, title: str, header_fill: str, row_fill_1: str, row_fill_2: str):
    sub_title(title)
    st.plotly_chart(
        make_table(
            df,
            header_fill=header_fill,
            row_fill_1=row_fill_1,
            row_fill_2=row_fill_2,
            first_col_width=230,
            other_col_width=112,
            height=300,
            value_formatter=fmt_pct_id0,
        ),
        use_container_width=True,
        config={"displayModeBar": True, "displaylogo": False},
    )
...
with tab_pdb:
    ...
    if pdb_tables is not None:
        render_growth_table(
            ...
            header_fill=SUCCESS,
            row_fill_1=CELL_GREEN_A,
            row_fill_2=CELL_GREEN_B,
        )
        render_growth_table(
            pdb_tables.get("qtq", empty_df("pdb")),
            "Tabel Quarter to Quarter (QtQ)",
            header_fill=PURPLE,
            row_fill_1=CELL_PURPLE_A,
            row_fill_2=CELL_PURPLE_B,
        )

    st.markdown("<div class='chart-note'>Format angka dashboard menggunakan gaya Indonesia tanpa desimal. Tabel nominal menampilkan proyeksi 2026, tabel YoY menampilkan pertumbuhan terhadap kuartal yang sama tahun sebelumnya, dan tabel QtQ menampilkan pertumbuhan terhadap kuartal sebelumnya. Kolom Full Year pada QtQ sengaja dibiarkan kosong karena tidak memiliki definisi baku yang setara dengan pertumbuhan tahunan.</div>", unsafe_allow_html=True)
    ...
    with ch2:
        st.plotly_chart(
            make_growth_chart(
                ...
                colors=[SUCCESS, ACCENT, PRIMARY, PURPLE, "#D14D72", "#F4A261"],
            ),
            use_container_width=True,
            config={"displayModeBar": True, "displaylogo": False},
        )
    with ch3:
        st.plotly_chart(
            make_growth_chart(
                ...
                colors=[PURPLE, SUCCESS, PRIMARY, ACCENT, "#D14D72", "#F4A261"],
            ),
            use_container_width=True,
            config={"displayModeBar": True, "displaylogo": False},
        )
...
if show_preview:
    with st.expander("Preview data yang berhasil dimuat", expanded=False):
        ...
        preview_keys = [
            ...
            workbook["makro"],
            workbook["pdb"],
            pdb_tables.get("yoy", empty_df("pdb")) if pdb_tables else empty_df("pdb"),
            pdb_tables.get("qtq", empty_df("pdb")) if pdb_tables else empty_df("pdb"),
            workbook["moneter"],
            workbook["fiskal"],
        ]

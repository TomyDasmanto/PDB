...
st.markdown(
    ...
    <style>
        ...
        .sticky-table-wrap {{ width: 100%; overflow: visible; overflow-y: visible; margin: 0.2rem 0 0.6rem 0; border-radius: 12px; }}
        .sticky-table {{ border-collapse: separate; border-spacing: 0; width: max-content; min-width: 100%; font-size: 0.95rem; }}
        .sticky-table thead th {{ position: sticky; top: 0; z-index: 4; color: white; font-weight: 700; white-space: nowrap; }}
        .sticky-table th, .sticky-table td {{ padding: 8px 10px; border-right: 1px solid rgba(255,255,255,0.45); border-bottom: 1px solid rgba(255,255,255,0.45); vertical-align: middle; }}
        .sticky-table th:first-child, .sticky-table td:first-child {{ position: sticky; left: 0; min-width: 260px; max-width: 360px; white-space: normal; word-break: break-word; }}
        .sticky-table thead th:first-child {{ z-index: 6; }}
        .sticky-table tbody td:first-child {{ z-index: 3; }}
        .sticky-table .num {{ text-align: center; white-space: nowrap; min-width: 92px; }}
        .sticky-table .txt {{ text-align: left; }}
        .fiscal-editor-header {{ display: block; margin-top: 0.35rem; margin-bottom: 0.25rem; }}
        .fiscal-editor-title {{ color: {PRIMARY}; font-size: 1.02rem; font-weight: 700; line-height: 1.2; display: inline; }}
        .fiscal-editor-unit {{ color: #111827; font-size: 0.92rem; line-height: 1.2; display: inline; margin-left: 0.35rem; }}
        .simfiskal-editor-wrap {{ width: fit-content; max-width: 100%; overflow: visible; }}
...
def format_display(df: pd.DataFrame, value_formatter=fmt_id0) -> pd.DataFrame:
    ...
    view["Indikator"] = view["Indikator"].apply(lambda x: wrap_table_label(x, width=18))

...
BLOCK_NOTES = {
    ...
    "pdb": "Tabel accounting/PDB diisi dari data historis kuartalan. Outlook Q1–Q4 memakai angka tahun 2026, sedangkan Baseline memakai total tahun 2025 dan Full Year memakai total tahun 2026.",
    "moneter": "Tabel variabel moneter per periode.",
    "fiskal": "Tabel komponen fiskal per periode.",
}

RAW_REQUIRED = [
    "Konsumsi RT", "Konsumsi LNPRT", "PKP", "PMTB", "Ekspor", "Impor", "Change in Stocks"
]
...
def safe_excel_file(file_bytes: bytes):
    return pd.ExcelFile(BytesIO(file_bytes), engine="openpyxl")


def derive_pdb_from_realisasi(file_bytes: bytes):
    xls = safe_excel_file(file_bytes)
    ...
    required = [c for c in RAW_REQUIRED if c in df.columns]
    if len(required) < len(RAW_REQUIRED):
        return empty_df("pdb"), None

    df["C"] = pd.to_numeric(df["Konsumsi RT"], errors="coerce") + pd.to_numeric(df["Konsumsi LNPRT"], errors="coerce")
    df["I"] = (
        pd.to_numeric(df["PKP"], errors="coerce")
        + pd.to_numeric(df["PMTB"], errors="coerce")
        + pd.to_numeric(df["Change in Stocks"], errors="coerce")
    )
    df["X"] = pd.to_numeric(df["Ekspor"], errors="coerce")
    df["M"] = pd.to_numeric(df["Impor"], errors="coerce")
    df = df[["tanggal", "C", "I", "X", "M"]].copy()
    ...
    table = pd.DataFrame({"indikator": ["C", "I", "X", "M"]})
    baseline_map = data_2025[["C", "I", "X", "M"]].sum().to_dict() if not data_2025.empty else {}
    full_year_map = data_2026[["C", "I", "X", "M"]].sum().to_dict() if not data_2026.empty else {}

    quarter_name_map = {1: "out_tw1", 2: "out_tw2", 3: "out_tw3", 4: "out_tw4"}
    for col in ["baseline", "out_tw1", "out_tw2", "out_tw3", "out_tw4", "full_year"]:
        table[col] = None

    for idx, row in table.iterrows():
        comp = row["indikator"]
        table.at[idx, "baseline"] = baseline_map.get(comp)
        table.at[idx, "full_year"] = full_year_map.get(comp)
        for q in [1, 2, 3, 4]:
            qdf = data_2026[data_2026["quarter"] == q]
            if not qdf.empty:
                table.at[idx, quarter_name_map[q]] = qdf.iloc[0][comp]

    hist = df.melt(id_vars="tanggal", value_vars=["C", "I", "X", "M"], var_name="komponen", value_name="nilai")
...
def load_dashboard_data(uploaded_file):
    ...
    if uploaded_file is None:
        return data, hist_pdb

    file_bytes = uploaded_file.getvalue()
    xls = safe_excel_file(file_bytes)
    lower_sheet_map = {s.lower().strip(): s for s in xls.sheet_names}

    # generic blocks if workbook already has dedicated sheets
    ...
    # pdb: prioritize special derivation from realisasi sheet
    if "realisasi" in lower_sheet_map:
        data["pdb"], hist_pdb = derive_pdb_from_realisasi(file_bytes)
...
def make_pdb_history_chart(hist_df: pd.DataFrame, selected_components: list[str]):
    if hist_df is None or hist_df.empty:
        fig = go.Figure()
        fig.add_annotation(
            text="Data historis PDB belum tersedia pada file yang diunggah.",
    ...
    fig = px.line(
        ...
        color="komponen",
        markers=True,
        color_discrete_sequence=[PRIMARY, ACCENT, "#2A9D8F", "#8A5CF6"],
    )
    fig.update_traces(
        mode="lines+markers",
        line=dict(width=2.8),
...
workbook, pdb_history = load_dashboard_data(uploaded)
...
with tab_pdb:
    ...
    st.markdown("<div class='chart-note'>Grafik historis memakai komponen turunan: C = Konsumsi RT + Konsumsi LNPRT, I = PKP + PMTB + Change in Stocks, X = Ekspor, M = Impor.</div>", unsafe_allow_html=True)
    component_options = ["C", "I", "X", "M"]
    ...
    selected_components = selected_components or component_options
    st.plotly_chart(
...
with st.expander("Lihat struktur sheet Excel yang diharapkan"):
    info = pd.DataFrame({
        "Sheet": ["realisasi / simulasi / makro / moneter / fiskal"],
        "Keterangan": ["Jika tersedia sheet 'realisasi', maka blok Accounting / PDB akan otomatis dibentuk dari data historis dan outlook 2026."],
    })
...
if show_preview:
    with st.expander("Preview data yang berhasil dimuat", expanded=False):
        ...
        if pdb_history is not None:
            st.markdown("### Preview historis PDB turunan")
